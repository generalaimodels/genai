...
- RMSNorm, MLP-projections, and FairScale model parallel-compatible layers
- Dynamic positional embeddings for vision streams
- Scalable transformer blocks supporting large-batch training and efficient inference
- Extensible hooks for normalization, quantization, and LoRA/MoE integration

Design goals:
- Clean abstractions across modalities (vision/text), easy to extend with adapters
- Efficient long-sequence inference using KV cache and temperature tuning
- Compatibility with FairScale model-parallel Column/Row-parallel layers
- Robust validation for VisionArgs/ModelArgs (pydantic-powered)
- Rich-powered verbose metadata printing (opt-in) for interactive exploration

Important fixes:
- Fixed FairScale initialization check - now properly falls back to nn.Linear when FairScale is not initialized
- Fixed scatter_embeddings tensor dimension expansion issue
- Fixed BF16 dtype consistency in attention mechanism (temperature tuning and KV cache)
- Removed forced vision BF16 casting; all modules now start in float32 by default
- Images are cast to the text hidden dtype during fusion
- Vision adapter outputs are cast to the text dtype before scatter to avoid dtype mismatches

How to enable verbose rich-meta:
- Set environment variable MULTIMODAL_VERBOSE=1 or toggle VERBOSE=True in examples below.
- This prints shapes, configs, dtypes, and step summaries while models run.
"""
...
def _is_fairscale_initialized() -> bool:
    """Check if FairScale model parallel is properly initialized."""
    if not _FAIRSCALE_AVAILABLE:
        return False
    try:
        # Try to get the model parallel group - this will fail if not initialized
...
# ------------------------------------------------------------------------------
# 1b) FairScale fallback layers: ColumnParallelLinear, RowParallelLinear
# ------------------------------------------------------------------------------

class ColumnParallelLinear(nn.Module):
    """
    FairScale-compatible shim. Uses nn.Linear when FairScale is unavailable or not initialized.
    Arguments mimic FairScale's signature; unused args are accepted for compatibility.
    """
    def __init__(
        self,
        in_features: int,
        ...
        **kwargs: Any,
        ...
        # Check if FairScale is available AND properly initialized
        use_fairscale = _FAIRSCALE_AVAILABLE and _is_fairscale_initialized()
        ...
        if use_fairscale:
            try:
                ...
                # Fallback to regular linear if FairScale fails
                rich_print(f"[yellow]FairScale ColumnParallelLinear failed, using nn.Linear: {e}[/yellow]")
        ...
        **kwargs: Any,
        ...
        if use_fairscale:
            try:
                ...
                # Fallback to regular linear if FairScale fails
                rich_print(f"[yellow]FairScale RowParallelLinear failed, using nn.Linear: {e}[/yellow]")
                self.layer = nn.Linear(in_features, out_features, bias=bias)
                if init_method:
                    with torch.no_grad():
                        init_method(self.layer.weight)
...
# Safe wrapper for FairScale functions
def get_model_parallel_world_size() -> int:
    """Safe wrapper for getting model parallel world size."""
    if _is_fairscale_initialized():
        return fs_init.get_model_parallel_world_size()
    return 1

def get_model_parallel_rank() -> int:
    """Safe wrapper for getting model parallel rank."""
    if _is_fairscale_initialized():
        return fs_init.get_model_parallel_rank()
    return 0

# ------------------------------------------------------------------------------
# 2) Pydantic Arg Models + Validation
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 4) Vision pipeline: patching + pixel shuffle + MLP
# ------------------------------------------------------------------------------
...
class Attention(nn.Module):
    def __init__(
        ...
        world_size = get_model_parallel_world_size()  # Use safe wrapper
    ...
    def forward(
        ...
        # FIXED: Temperature tuning with consistent dtype
        if self.attn_temperature_tuning and not self.use_rope:
            # Use the same dtype as the query tensor to avoid dtype promotion
            seq_positions = torch.arange(start_pos, start_pos + seqlen, device=xq.device, dtype=xq.dtype)
        ...
        # FIXED: Ensure KV cache operations preserve dtype
        self.cache_k[:bsz, start_pos : start_pos + seqlen] = xk.to(self.cache_k.dtype)
        self.cache_v[:bsz, start_pos : start_pos + seqlen] = xv.to(self.cache_v.dtype)
        ...
        # FIXED: Ensure all tensors have the same dtype before attention
...
class VisionEncoder(nn.Module):
    def __init__(
        ...
        self.packed_img_idx = packed_img_idx
        ...
        self.freq_cis = torch.view_as_complex(torch.stack([torch.cos(freqs), torch.sin(freqs)], dim=-1)).squeeze(0)
...
# ------------------------------------------------------------------------------
# 8) Vision Embeddings + scatter back to token sequence
# ------------------------------------------------------------------------------
...
def scatter_embeddings(
    ...
    """
    Scatter encoded image patches into text sequence positions.
    
    Args:
        image_batch: List of batched images
        image_mask: Boolean mask of shape [B, T] indicating image positions
        h_image: Target tensor of shape [B, T, D] to scatter into
        encoded_patches_proj: Encoded image patches to scatter
    
    Returns:
        Updated h_image with scattered embeddings
    """
    ...
    for index in range(h_image.size(0)):
        ...
        sample_image_mask = image_mask[index]  # Shape: [T]
        ...
        # FIXED: Properly expand the mask for masked_scatter_
        # sample_image_mask is [T], we need [T, D] for masked_scatter_
        expanded_mask = sample_image_mask.unsqueeze(-1).expand(-1, h_image.size(-1))
...
class VisionEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # Align image tensor to the text hidden dtype/device to avoid dtype mismatches
        images_flattened = torch.vstack(images_flattened).unsqueeze(1).to(h_ref.dtype).to(h_ref.device)
        embedding = self.vision_encoder(images_flattened)        # [sum_tiles, N_patches, dim]
        projected_embedding = self.vision_adapter(embedding)     # [sum_tiles, N_proj_patches, output_dim]

        h_image = self._get_empty_sequence(h_ref)
        # Ensure scatter source matches target dtype (fixes masked_scatter dtype issues)
        projected_embedding = projected_embedding.to(h_image.dtype)
        return scatter_embeddings(image_batch, image_mask, h_image, projected_embedding)

# ------------------------------------------------------------------------------
# 9) Text stack + Multimodal model wrapper
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 11) Utilities: parameter counting, dtype/device reports
# ------------------------------------------------------------------------------
...
def model_summary(model: nn.Module, name: str = "Model") -> None:
    ...
    kv = {
        ...
        "fairscale": _FAIRSCALE_AVAILABLE,
        "fairscale_initialized": _is_fairscale_initialized(),
        "mp world_size": get_model_parallel_world_size(),
    }
...
# ------------------------------------------------------------------------------
# 12) Examples: Training, Decoding, and BF16 end-to-end
# ------------------------------------------------------------------------------
...
def _example_train_step():
    ...
    tokens, image_batch, image_mask = _example_synthetic_batch(
        bsz=2, T=64, vocab_size=args.vocab_size, with_vision=True, device=device,
        dtype=torch.float32  # keep FP32 for stability across devices
    )
...
def _example_bf16_end2end():
    """
    Optional: Run everything in BF16 (CUDA recommended). Ensures consistent dtype across modules.
    """
    global VERBOSE
    VERBOSE = True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if device.type != "cuda":
        rich_print("[red]BF16 example is best on CUDA; falling back to FP32 CPU.[/red]")
        return
